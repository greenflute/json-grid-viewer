<template>
  ...
  <span v-else class="value-inline hover-edit-anchor hover-edit-right" :class="{ editing }">
    <span
      v-if="!editing"
      ref="displayValue"
      :class="['value', (element === null ? 'null' : typeof element)]"
    >{{ displayValue }}</span>
    <span v-else class="inline-editor" :class="{ multiline: useTextarea }">
      <span v-if="useTextarea" class="textarea-overlay">
        <pre ref="previewLayer" class="json-textarea-preview" aria-hidden="true"><span v-for="(line, index) in previewLines" :key="`${index}:${line.text}`" class="preview-line">{{ line.text || '\u200b' }}<span v-if="line.hasBreak" class="hard-break-marker">↵</span></span></pre>
        <textarea
          ref="editorInput"
          v-model="draft"
          class="json-textarea json-textarea-overlay"
...
export default {
  ...
  computed: {
    ...
    displayValue() {
      return this.element === null ? '' : String(this.element)
    },
    useTextarea() {
      return this.preferTextarea || (typeof this.element === 'string' && this.element.includes('\n'))
    },
    singleLineInputStyle() {
      const width = `${Math.max(this.draft.length + 1, 8)}ch`
      return {
        width,
        maxWidth: 'min(42rem, calc(100vw - 3rem))'
      }
    },
    previewLines() {
  ...
  methods: {
    ...
    toggleEdit() {
      ...
      this.preferTextarea = this.editing ? this.shouldUseTextareaEditor() : false
      if (this.editing) {
        this.$nextTick(() => {
    ...
    shouldUseTextareaEditor() {
      if (typeof this.element === 'string' && this.element.includes('\n')) {
        return true
      }

      const valueEl = this.$refs.displayValue
      if (!valueEl) {
        return false
      }

      const overflowX = valueEl.scrollWidth - valueEl.clientWidth > 1
      const overflowY = valueEl.scrollHeight - valueEl.clientHeight > 1
